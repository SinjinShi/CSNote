...
&& 与 || 属于短路运算，当第一项值不满足期望时，不会继续判断后续的项，是一种很好的「容错、容灾、降级」方案，同时也可以用于提高性能表现（将常见的判断放到第一项，减少不必要的判断）
...
（2）字符串的比较：
1. 不会进行数值转换，比较字符串每一位的 Unicode 编码
2. 一位一位进行比较进行比较， "5" > "123"
...
NaN 不和任何值相等，包括他本身。可以使用 `isNaN()` 进行判断
